...
def get_CPU_mem():
    import psutil
    return psutil.virtual_memory().used / 1024 / 1024 / 1024  # GB 단위

def get_GPU_mem():
    if torch.cuda.is_available():
        return [torch.cuda.memory_allocated(i) / 1024 / 1024 / 1024 for i in range(torch.cuda.device_count())]
    else:
        return [0.0]
...
class Trainer(object):
    def __init__(self,
        ...
        # after: if self.opt.images is not None: self.pearson = ...
        self.vgg = None
        if getattr(self.opt, 'lambda_perc', 0.0) > 0:
            vgg = models.vgg16(weights=models.VGG16_Weights.IMAGENET1K_FEATURES).features.eval().to(self.device)
            for p in vgg.parameters():
                p.requires_grad = False
            # conv1_2, conv2_2, conv3_3의 출력까지 뽑기
            self.vgg_layers = [3, 8, 15]  # relu1_2, relu2_2, relu3_3
            self.vgg = vgg
        ...
        self.loss_csv_header_written = False

        if self.workspace is not None and self.local_rank == 0:
            self.loss_csv_path = os.path.join(self.workspace, "loss_log.csv")
            if not os.path.exists(self.loss_csv_path):
                with open(self.loss_csv_path, "w", newline="") as f:
                    writer = csv.writer(f)
                    writer.writerow(["epoch", "global_step", "phase", "loss"])
                self.loss_csv_header_written = True
            else:
                self.loss_csv_header_written = True
    ...
    def train_step(self, data, save_guidance_path:Path=None):
        ...
        if self.workspace is not None and self.local_rank == 0 and (self.global_step % 50 == 0):
            save_dir = os.path.join(self.workspace, "results")
            os.makedirs(save_dir, exist_ok=True)

            m = pred_mask[0, 0].detach().clamp(0, 1).cpu().numpy()  # [H,W]
            m = (m * 255).astype(np.uint8)

            cv2.imwrite(os.path.join(save_dir, f"mask_{self.global_step:07d}.png"), m)
        ...
        # [NEW] Exp-I: SDS gradient reweight (only for SDS phase)
        if (not do_rgbd_loss) and getattr(self.opt, 'mask_weighted_sds', False):
            seam_mask = data.get('seam_mask', None)  # [B,1,H,W] or None
            edge_mask = self._sobel_edge(pred_rgb)   # [B,1,H,W]
            if seam_mask is None:
                seam_mask = torch.zeros_like(edge_mask)

            W = 1.0 + self.opt.alpha_sds * seam_mask + self.opt.beta_sds * edge_mask
            ...
            pred_rgb.register_hook(_hook_weight)
        ...
        # novel view loss
            ...
            # L1) Seam continuity
            if getattr(self.opt, 'lambda_seam', 0.0) > 0:
                loss_seam = 0.0
                seam_mask = data.get('seam_mask', None)  # [B,1,H,W]
                if seam_mask is not None:
                    gx = pred_rgb[:, :, :, 1:] - pred_rgb[:, :, :, :-1]
                    gy = pred_rgb[:, :, 1:, :] - pred_rgb[:, :, :-1, :]
                    gx = F.pad(gx, (0, 1, 0, 0))
                    gy = F.pad(gy, (0, 0, 0, 1))
                    loss_seam += (seam_mask * gx.abs().mean(1, keepdim=True)).mean()
                    loss_seam += (seam_mask * gy.abs().mean(1, keepdim=True)).mean()

                seam_pairs = data.get('seam_pairs', None)  # list of (ip,im)
                if seam_pairs:
                    B_, C_, H_, W_ = pred_rgb.shape
                    pr = pred_rgb.view(B_, C_, H_ * W_)
                    l = 0.0
                    for (ip, im) in seam_pairs:
                        ip = torch.as_tensor(ip, device=pred_rgb.device, dtype=torch.long)
                        im = torch.as_tensor(im, device=pred_rgb.device, dtype=torch.long)
                        l += (pr[:, :, ip] - pr[:, :, im]).abs().mean()
                    loss_seam += l / max(1, len(seam_pairs))

                loss = loss + self.opt.lambda_seam * loss_seam

            # L2) Perceptual (VGG)
            if getattr(self.opt, 'lambda_perc', 0.0) > 0 and self.vgg is not None:
                I_pred = pred_rgb.clamp(0, 1)
                I_ref = None
                if 'I_ref' in data:
                    I_ref = data['I_ref'].to(I_pred.device).clamp(0, 1)
                elif do_rgbd_loss:
                    I_ref = gt_rgb.clamp(0, 1)
                if I_ref is not None:
                    f_p, f_r = self._vgg_feats(I_pred), self._vgg_feats(I_ref)
                    if len(f_p) == len(f_r) and f_p:
                        loss_perc = sum(F.mse_loss(a, b) for a, b in zip(f_p, f_r))
                        loss = loss + self.opt.lambda_perc * loss_perc

            # L3) Chromaticity (Lab a,b)
            if getattr(self.opt, 'lambda_chroma', 0.0) > 0:
                I_rgb = pred_rgb[:, :3, :, :] if pred_rgb.shape[1] > 3 else pred_rgb
                ab = self._rgb_to_lab_ab(I_rgb.clamp(0, 1))
                if ab is not None:
                    mu = ab.mean(dim=0, keepdim=True)
                    loss_chroma = (ab - mu).abs().mean()
                    loss = loss + self.opt.lambda_chroma * loss_chroma

            # I2) Normal-guided refinement
            if getattr(self.opt, 'lambda_norm_refine', 0.0) > 0 and 'normal_image' in outputs:
                pn = 2 * pred_normal - 1                 # [-1,1]
                pn_bchw = pn.permute(0, 3, 1, 2).contiguous()
                pn_s = TF.gaussian_blur(pn_bchw, kernel_size=9)
                cos = F.cosine_similarity(pn_bchw, pn_s, dim=1).mean()
                loss = loss + self.opt.lambda_norm_refine * (1 - cos)
    ...
    def save_mesh(self, loader=None, save_path=None):
        ...
        if hasattr(self.model, "update_extra_state"):
            with torch.cuda.amp.autocast(enabled=self.fp16):
                self.model.update_extra_state()
        is_mesh_only_vanilla = (
            getattr(self.opt, "backbone", "grid") == "vanilla"
            and getattr(self.opt, "lambda_guidance", 1.0) == 0.0
            and getattr(self.opt, "lambda_rgb", 1000.0) == 0.0
            and self.opt.iters <= 1
        )

        mesh = None

        try:
            if is_mesh_only_vanilla:
                self.log("[INFO] mesh-only vanilla mode: export_mesh(resolution)")
                mesh = self.model.export_mesh(
                    self.opt.mcubes_resolution
                )
            else:
                self.log("[INFO] standard mode: export_mesh(save_path, resolution, threshold)")
                mesh = self.model.export_mesh(
                    save_path,
                    resolution=self.opt.mcubes_resolution,
                    threshold=self.opt.density_thresh,
                )
        except TypeError as e:
            self.log(f"[WARN] export_mesh call TypeError: {e}, trying fallback without threshold...")
            try:
                mesh = self.model.export_mesh(
                    resolution=self.opt.mcubes_resolution,
                )
            except Exception as ee:
                self.log(f"[ERROR] fallback export_mesh(resolution=..) also failed: {ee}")
                return
        except Exception as e:
            self.log(f"[ERROR] export_mesh call failed: {e}")
            return
        try:
            import trimesh
            if isinstance(mesh, trimesh.Trimesh):
                out_path = os.path.join(save_path, "mesh.obj")
                mesh.export(out_path)
                self.log(f"[INFO] Mesh exported successfully to {out_path}")
            else:
                self.log(f"[INFO] export_mesh returned {type(mesh)}, assume it handled saving internally.")
        except Exception as e:
            self.log(f"[WARN] mesh export/save failed: {e}")
    ...
    def train_one_epoch(self, loader, max_epochs):
        ...
        self._append_loss_csv(epoch=self.epoch,
    ...
    def _append_loss_csv(self, epoch, global_step, phase, loss_value: float):
        if self.loss_csv_path is None or self.local_rank != 0:
            return
        try:
            with open(self.loss_csv_path, "a", newline="") as f:
                writer = csv.writer(f)
                writer.writerow([int(epoch), int(global_step), phase, float(loss_value)])
        except Exception as e:
            self.log(f"[WARN] failed to write loss csv: {e}")

    
    
    def _vgg_feats(self, img_bchw):
            # img_bchw: [B,3,H,W] in [0,1]
            x = (img_bchw - 0.5) / 0.5
            feats = []
            if self.vgg is None:
                return feats
            h = x
            for li, layer in enumerate(self.vgg):
                h = layer(h)
                if li in self.vgg_layers:
                    feats.append(h)
            return feats

    def _rgb_to_lab_ab(self, img_bchw):
        # returns ab: [B,2,H,W]
        if kornia is None:
            return None
        lab = Kcolor.rgb_to_lab(img_bchw.clamp(0,1))
        return lab[:,1:3]  # a,b

    def _sobel_edge(self, img_bchw):
        gx = img_bchw[:,:,:,1:] - img_bchw[:,:,:,:-1]
        gy = img_bchw[:,:,1:,:] - img_bchw[:,:,:-1,:]
        # padding to original H,W
        gx = F.pad(gx, (0,1,0,0))
        gy = F.pad(gy, (0,0,0,1))
        mag = torch.sqrt((gx**2 + gy**2).mean(1, keepdim=True) + 1e-8)
        mag = mag / (mag.amax(dim=[2,3], keepdim=True) + 1e-8)
        return mag

    def _plot_loss_curve(self):
        """loss_log.csv를 읽어서 epoch별 train/val loss를 PNG로 저장."""
        if self.loss_csv_path is None or self.local_rank != 0:
            return
        try:
            import csv
            ...
            with open(self.loss_csv_path, "r") as f:
            ...
            if len(epochs_tr) == 0 and len(epochs_val) == 0:
                self.log("[WARN] no data in loss_log.csv to plot.")
                return
            plt.figure()
            if epochs_tr:
            ...
            out_path = os.path.join(self.workspace, "loss_curve.png")
            plt.savefig(out_path, dpi=150, bbox_inches="tight")
            plt.close()
            self.log(f"[INFO] saved loss curve to {out_path}")
        except Exception as e:
            self.log(f"[WARN] failed to plot loss curve: {e}")
