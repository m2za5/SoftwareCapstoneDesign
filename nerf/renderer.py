...
print("### RENDERER VERSION: 2025-12-17 shell-f32-b16384 ###")
...
class NeRFRenderer(nn.Module):
    ...
    @torch.no_grad()
    def init_from_mesh(self, mesh, resolution=96, sigma_scale=10.0):
        import numpy as np
        import trimesh
        import time
        import sys
        ...
        print(f"[init_from_mesh] Checking mesh complexity... Current faces: {n_faces_before}")

        if n_faces_before > target_faces:
            print(f"[init_from_mesh] Too heavy! Decimating to {target_faces} faces...")
            try:
                mesh = mesh.simplify_quadratic_decimation(target_faces)
                n_faces_after = len(mesh.faces)
                print(f"[init_from_mesh] Decimation Result: {n_faces_before} -> {n_faces_after}")
                if n_faces_after > 80000:
                     raise RuntimeError(f"Decimation ineffective! Still has {n_faces_after} faces. Aborting to save time.")
                     
            except Exception as e:
                print(f"[init_from_mesh] Decimation Failed or Error: {e}")
                raise RuntimeError("Decimation failed. Cannot proceed with high resolution density generation.")
        else:
             print(f"[init_from_mesh] Mesh is light enough. Skipping decimation.")
        cache_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "mesh_cache"))

        os.makedirs(cache_dir, exist_ok=True)
        meta = getattr(mesh, "metadata", {}) or {}
        mesh_path = meta.get("file_path", None)
        mesh_name = os.path.splitext(os.path.basename(mesh_path))[0] if mesh_path else "default"
        cache_path = os.path.join(cache_dir, f"{mesh_name}_{resolution}_b{self.bound}_filled.pt")
        
        if os.path.exists(cache_path):
            print(f"[init_from_mesh] Loading cached FILLED volume: {cache_path}")
            self.density_init = torch.load(cache_path).to(self.aabb_train.device)
            return

        print(f"[init_from_mesh] Generating FILLED volume (res={resolution})...")
        ...
        size = (bbox_max - bbox_min).max() + 1e-8
        scale = (2 * self.bound * 0.9) / size 
        mesh.vertices = (mesh.vertices - center) * scale

        xs = np.linspace(-self.bound, self.bound, resolution, dtype=np.float32)
        ys = np.linspace(-self.bound, self.bound, resolution, dtype=np.float32)
        zs = np.linspace(-self.bound, self.bound, resolution, dtype=np.float32)
        ...
        print(f"[init_from_mesh] Computing SDF for {total_pts} points...")
        
        loop_start = time.time()
        for i in range(0, total_pts, batch_size):
            batch_pts = pts[i:i+batch_size]
            d = proximity.signed_distance(batch_pts).astype(np.float32)
            dists.append(d)

            current_count = min(i + batch_size, total_pts)
            percent = (current_count / total_pts) * 100
            
            elapsed = time.time() - loop_start
            if elapsed > 0 and current_count > 0:
                speed = current_count / elapsed 
                remaining = total_pts - current_count
                eta = remaining / speed
                sys.stdout.write(f"\r   >> Progress: {percent:.1f}% | ETA: {eta:.1f}s | Speed: {speed:.0f} pts/s ")
                sys.stdout.flush()

        print("") 
        dists = np.concatenate(dists)

        width = 0.05 * self.bound 
        vol_density = np.zeros_like(dists)
        mask_inside = dists <= 0
        vol_density[mask_inside] = sigma_scale
        vol_density[~mask_inside] = sigma_scale * np.exp(-dists[~mask_inside] / width)
        vol_density = np.clip(vol_density, 0, sigma_scale).astype(np.float32)

        self.density_init = torch.from_numpy(vol_density.reshape(resolution, resolution, resolution)).to(self.aabb_train.device)
        
        end_time = time.time()
        print(f"[init_from_mesh] Completed in {end_time - total_start_time:.2f}s. Saved to {cache_path}")
        
        torch.save(self.density_init.cpu(), cache_path)
        return
    ...
    def export_mesh(self, path, resolution=None, decimate_target=-1, S=128):
        ...
        #temp_mesh = trimesh.Trimesh(vertices, triangles, process=False)
        #trimesh.smoothing.filter_laplacian(temp_mesh, iterations=10)
        ...
        mesh.export(os.path.join(path, "raw_mesh.ply"))
        mesh.export(os.path.join(path, "raw_mesh.obj"))
