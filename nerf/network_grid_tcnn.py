...
class NeRFNetwork(NeRFRenderer):
    ...
    def density(self, x):
        ...
        if hasattr(self, 'density_init'):
            sigma = sigma + 0.5 * self.density_init.mean().item()
    ...
    def init_from_mesh(self, mesh):
        ...
        print(f"[init_from_mesh] Received mesh with {len(mesh.vertices)} vertices / {len(mesh.faces)} faces")
        ...
        coords = ((verts + 1) / 2 * (grid_size - 1)).astype(int)
        coords = np.clip(coords, 0, grid_size - 1)
        density[coords[:,0], coords[:,1], coords[:,2]] = 1.0

        density = torch.from_numpy(density).unsqueeze(0).unsqueeze(0).float().to(next(self.parameters()).device)
        kernel = torch.tensor([[[[1.,2.,1.],[2.,4.,2.],[1.,2.,1.]]]], device=density.device)
        kernel /= kernel.sum()
        density = F.conv3d(density, kernel.expand(1,1,3,3,3), padding=1)
        density = density / density.max()

        # assign into internal density field
        if hasattr(self, 'density_grid'):
            self.density_grid.data = density[0,0]
            print(f"[init_from_mesh] density_grid updated to shape {self.density_grid.shape}")
        else:
            self.density_init = density
            print("[init_from_mesh] stored initial density tensor as self.density_init.")
